...
func (c *KCPClient) GetRuntimeCrOIDCConfig(instanceID string) (interface{}, error) {
	args := []string{"rt", "-i", instanceID, "--runtime-config", "-o", "custom=:{.runtimeConfig.spec.shoot.kubernetes.kubeAPIServer.additionalOidcConfig}"}
	if clientSecret := os.Getenv("KCP_OIDC_CLIENT_SECRET"); clientSecret != "" {
		args = append(args, "--config", "config.yaml")
	}
	output, err := exec.Command("kcp", args...).Output()
