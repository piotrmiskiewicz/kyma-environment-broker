...
type Builder struct {
	...
	kubeconfigProvider *SecretProvider
}

func NewBuilder(provisionerClient provisioner.Client, provider *SecretProvider) *Builder {
	return &Builder{
...
func (b *Builder) BuildFromAdminKubeconfig(instance *internal.Instance, adminKubeconfig string) (string, error) {
	...
	logrus.Info("Building Kubeconfig:")
	logrus.Infof(" - instance OIDS issuer/clientID: %s/%s", instance.Parameters.Parameters.OIDC.IssuerURL, instance.Parameters.Parameters.OIDC.ClientID)
	logrus.Infof(" - clusterconfig (Provisioner) issuer/clientID: %s/%s", status.RuntimeConfiguration.ClusterConfig.OidcConfig.IssuerURL, status.RuntimeConfiguration.ClusterConfig.OidcConfig.ClientID)
